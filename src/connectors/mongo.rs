...
impl MongoClient {
    ...
    pub async fn gather_managerinfo(&mut self) -> Result<ManagerInfo> {
        let result = ManagerInfo {
            ...
            auto_ping_kick_count: self
                .logging
                .count_documents(bson::doc! {"action": "autokick-ping"}, None)
                .await
                .unwrap_or(0) as i64,
            bfban_count: self
                .logging
                .count_documents(bson::doc! {"action": "autokick-bfban"}, None)
                .await
                .unwrap_or(0) as i64,
            move_count: self
                .logging
                .count_documents(bson::doc! {"action": "movePlayer"}, None)
                .await
                .unwrap_or(0) as i64,
            kick_count: self
                .logging
                .count_documents(bson::doc! {"action": "kickPlayerg"}, None)
                .await
                .unwrap_or(0) as i64,
            ban_count: self
                .logging
                .count_documents(bson::doc! {"action": "addServerBan"}, None)
                .await
                .unwrap_or(0) as i64,
            global_ban_kick_count: self
                .logging
                .count_documents(bson::doc! {"action": "autokick-globalBans"}, None)
                .await
                .unwrap_or(0) as i64,
            timestamp: Utc::now(),
        };
    ...
    pub async fn push_totals(
        &mut self,
        _global_result: results::RegionResult,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    pub async fn gather_old_title(
        ...
        self.old_games_servers
            .find_one(bson::doc! {"_id": game_name}, None)
            .await
    }
}
